...
for (const url of ["https://crawlfoundry.com/features/keyword-database", "https://crawlfoundry.com/features/site-audit"]) if (!home.includes(`href="${url}?`)) failures.push("home must provide the relevant Crawl Foundry workflow handoff: " + url);
for (const disclosure of ["DevAwesome and Crawl Foundry are operated by Matthias Ramahi", "are not independent recommendations"]) if (!home.includes(disclosure)) failures.push("home must disclose common ownership beside Crawl Foundry workflow links: " + disclosure);
